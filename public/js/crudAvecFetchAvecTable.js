...
function getRequest4() {
    let url = "/api/restaurants/56b9f89be0adc7f00f348d02";
...
function afficheReponseGET(reponse) {
    ...
    let restaurants = [];
    // check is reponse.data is an array
    if (Array.isArray(reponse.data)) {  
         restaurants = reponse.data;
    } else {
         restaurants.push(reponse.data);
    }
    afficheRestaurantsEnTable(restaurants);
}
...
function afficheRestaurantsEnTable(restaurants) {
    ...
    console.log("creer tableau nb restaurants = " + restaurants.length);
